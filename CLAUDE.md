...
- **Always prefix `bash -n <file>` in the `-- test:` for any step that edits a bash script**, before any grep checks: `bash -n loop.sh && grep -q 'key-string' loop.sh`. Free providers frequently introduce unbalanced `if/fi` or `{/}` that pass the grep check but break the file. `bash -n` catches this immediately and puts the exact line number into `last-test-error.txt` for the next prompt.
- **For bash function edits, include the full test suite in `-- test:`**: `(cd project && bash test-engine.sh) | grep -q 'passed, 0 failed'`. Syntax checks alone miss structural problems (orphaned code, duplicate lines, dropped variable declarations) that are syntactically valid but functionally broken.
- **Set `-- files:` ranges to show the full function being modified**, not just the target lines. Include a few lines past the closing `}` or `fi` so providers can see the complete brace structure and don't guess wrong.
