...
## Steps
- [x] Add bash-side task close to loop.sh and project/loop.sh: in the `RESULT=pass` handler (after the recovery file cleanup at ~line 361), if `$TASK_FILE` still exists and has no remaining unchecked steps (`! grep -q '^\- \[ \]' "$TASK_FILE"`), move it to `tasks/3_done/YYYY-MM-DD-name.md`, append to `CHANGELOG.md`, commit the admin files (`git add tasks/3_done/ CHANGELOG.md && git commit -m "${CURRENT_TASK}: close task"`), then run the iter/error cleanup. Also update the existing auto-close block (lines 267-275) to include the same git commit and cleanup. Make identical changes to project/loop.sh — done when: both files contain the string `close task` in a commit message — test: bash -n loop.sh && bash -n project/loop.sh && grep -q 'close task' loop.sh && grep -q 'close task' project/loop.sh -- files: loop.sh:265-280, loop.sh:358-375, project/loop.sh:265-280, project/loop.sh:358-375
- [ ] Update prompt.md: remove step 3 (the "if final step, move file + append CHANGELOG" block). Keep steps 1-2 (mark step done, commit) and the ARCHITECTURE_REVIEW instruction. Renumber so instructions stay clean — done when: prompt.md contains no reference to `tasks/3_done` — test: bash -n loop.sh && ! grep -q '3_done' prompt.md -- files: prompt.md, loop.sh:165-183
