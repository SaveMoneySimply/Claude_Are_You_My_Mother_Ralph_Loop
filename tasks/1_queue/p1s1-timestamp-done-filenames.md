...
- [ ] Step 3: Edit the stale-task guard in `loop.sh` (~line 139) — same change: `if [ -f "tasks/3_done/${task_name}.md" ]` → `if compgen -G "tasks/3_done/*-${task_name}.md" > /dev/null 2>&1` — done when: guard uses compgen or glob -- test: grep -q 'compgen.*3_done.*task_name\|3_done/\*.*task_name' loop.sh && bash -n loop.sh

- [ ] Step 4: Edit `prompt.md` line 7 — change `mv tasks/2_active/<name>.md tasks/3_done/<name>.md` to `mv tasks/2_active/<name>.md "tasks/3_done/$(date +%Y-%m-%d)-<name>.md"` — done when: prompt.md references `date +%Y-%m-%d` -- test: grep -q 'date +%Y-%m-%d' prompt.md
