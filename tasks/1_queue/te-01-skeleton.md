...
- [ ] Add summary and exit: after all tests, print "SUMMARY: $PASS_COUNT passed, $FAIL_COUNT failed" and exit 1 if FAIL_COUNT > 0; also print a header line at the top of output like "=== Ralph test-engine ===" — done when: (cd project && bash test-engine.sh) prints SUMMARY line and exits 0 -- test: (cd project && bash test-engine.sh) | grep -q 'SUMMARY:'
